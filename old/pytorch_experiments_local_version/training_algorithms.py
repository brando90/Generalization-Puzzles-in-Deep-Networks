...
class StatsCollector:
    ...
    def collect_stats(self, i, mdl, Xtr,Ytr,Xv,Yv,Xt,Yt):
        # TODO do we really need it to be like this?
        ''' log train losses '''
...
def collect_final_activation(net,dataloader,enable_cuda):
    '''
        collect values for final activations
    '''
    running_loss,running_error = 0,0
    for i,data in enumerate(dataloader):
        inputs, labels = extract_data(enable_cuda,data,wrap_in_variable=True)
        outputs = net(inputs)
    return outputs[0,:] # should be 1 by 10 for cifar10
